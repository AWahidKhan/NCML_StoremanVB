...
Begin VB.Form Frm_MonthEmpProces 
   ...
   End
End
Attribute VB_Name = "Frm_MonthEmpProces"
...
For I = 1 To RsMst_Employee.RecordCount
    ...
    'tmp = "Insert into Txn_MonthEmp_History"
    'tmp = tmp & " Select " & mMaxID & ",'" & mToDate & "',"
    'tmp = tmp & " EmployeeID,EmployeeNo,EmployeeName,DesignationID,SelfReporting,DirectReportingTo,ReportingToDRC,ConnectedToRAG,EmployeeEmailID,MobileNo, "
    'tmp = tmp & " Address,Flag,Concurrency,CreatedBy,CreatedDate,UpdatedDate,UpdatedBy,NAVCode,AllocateExpense,CMPAllocation,"
    'tmp = tmp & " BaseLocationID,0,0," & IIf(IsNull(mDOL), "Null", "'" & mDOL & "'") & "," & IIf(IsNull(mDOJ), "Null", "'" & mDOJ & "')&", FAGUpdated, FAGUpdatedDate) & " From Mst_Employee where EmployeeID=" & RsMst_Employee!EmployeeID
    
    
    tmp = "Insert into Txn_MonthEmp_History"
    tmp = tmp & " Select " & mMaxID & ",'" & mToDate & "',EmployeeID,EmployeeNo,EmployeeName,DesignationID,SelfReporting,DirectReportingTo,ReportingToDRC,ConnectedToRAG,EmployeeEmailID,MobileNo,"
    tmp = tmp & " Address,Flag,Concurrency,CreatedBy,CreatedDate,UpdatedDate,UpdatedBy,NAVCode,AllocateExpense,CMPAllocation,"
    tmp = tmp & " BaseLocationID,0,0," & IIf(IsNull(mDOL), "Null", "'" & mDOL & "'") & "," & IIf(IsNull(mDOJ), "Null", "'" & mDOJ & "'") & ",FAGUpdated,FAGUpdatedDate From Mst_Employee where EmployeeID=" & RsMst_Employee!EmployeeID
