...
Begin VB.Form Frm_UpdateCREntryNo 
   Caption         =   "Update CR Entry No"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   4680
   ...
   End
End
Attribute VB_Name = "Frm_UpdateCREntryNo"
...
For I = 2 To MSHFlexGrid3.Rows - 1
    If MSHFlexGrid3.TextMatrix(I, 13) <> "" Then
       ...
       If UBound(mstr) = 1 Then
          mCRID = mstr(0)
          mCRID = Replace(mCRID, "CR", "")
          mSMPrefix = mstr(1)
         
''          tmp = "Update Txn_NewCashReceiptTIDetail set DocNo='" & Trim(MSHFlexGrid3.TextMatrix(I, 14)) & "',EntryNo=" & Val(MSHFlexGrid3.TextMatrix(I, 15)) & " Where CRID=" & Val(mCRID) & " And SM_Prefix='" & Trim(mSMPrefix) & "' And"
''          tmp = tmp & " (ClientIDRow in (Select ClientIDRow from Mst_Client Where NAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "' or RNAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "') or DPID in (Select DepositoryID from Mst_Depository Where NAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "'))"
          
          tmp = "Update Txn_NewCashReceiptTIDetail Set DocNo='" & Trim(MSHFlexGrid3.TextMatrix(I, 14)) & "',EntryNo=" & Val(MSHFlexGrid3.TextMatrix(I, 15)) & ""
          tmp = tmp & " Where TIID in( Select TIID from  Txn_TIDetails Where TIID in (select TIID from Txn_NewCashReceiptTIDetail Where CRID=" & Val(mCRID) & " and SM_Prefix='" & Trim(mSMPrefix) & "') and NAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "') And Isnull(CRAAID,0)=0 And CRID=" & Val(mCRID) & " and SM_Prefix='" & Trim(mSMPrefix) & "' "

          Call TmpTable
         
          '--for Advace
          tmp = "Update Txn_NewCashReceiptClientDetail set DocNo='" & Trim(MSHFlexGrid3.TextMatrix(I, 14)) & "',EntryNo=" & Val(MSHFlexGrid3.TextMatrix(I, 15)) & " Where CRID=" & Val(mCRID) & " And SM_Prefix='" & Trim(mSMPrefix) & "' And Amount > 0  And "
          tmp = tmp & " (ClientIDRow in (Select ClientIDRow from Mst_Client Where NAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "' or RNAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "') or DPID in (Select DepositoryID from Mst_Depository Where NAVCode='" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "'))"
          tmp = tmp & " And AdvNAVCode  = '" & Trim(MSHFlexGrid3.TextMatrix(I, 3)) & "' And CRID=" & Val(mCRID) & " and SM_Prefix='" & Trim(mSMPrefix) & "'"
