...
Begin VB.Form Frm_ITVendorNAVExport 
   Caption         =   "IT Vendor NAV Export"
   ClientHeight    =   9690
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   12000
   ...
   End
End
Attribute VB_Name = "Frm_ITVendorNAVExport"
...
If Option1.Value = True Then
   strFilName = "\Navision\ITVendorNavUpload_SMClaim" & MaxID & ".csv"
Else
   strFilName = "\Navision\ITVendorNavUpload_SMClaim" & CmbBatch.Text & ".csv"
End If
...
Next
If Option1.Value = True Then
   tmp = "Update Txn_ITVendorClaim Set BatchID = " & MaxID & " Where BatchID = 0 and  Flag = 'A'"
   Call TmpTable
End If
...
If Option1.Value = True Then
        ...
        tmp = tmp & " '' 'EmployeeNAVCode', VC.BatchID,CD.InvoiceDate  From Txn_ITVendorClaimDetail CD "
        tmp = tmp & " Inner Join Txn_ITVendorClaim VC on CD.ClaimID = VC.ClaimID  "
        tmp = tmp & " Inner Join Mst_ITVendor E on VC.VendorID = E.VendorID "
...
Else
   ...
   End If
        ...
        tmp = tmp & " L.NAVCode 'LocationNAVCode','CMP' As DeptDim , "
        tmp = tmp & " '' 'VendorNAVCode', VC.BatchID,CD.InvoiceDate  From Txn_ITVendorClaimDetail CD "
        tmp = tmp & " Inner Join Txn_ITVendorClaim VC on CD.ClaimID = VC.ClaimID  "
        tmp = tmp & " Inner Join Mst_ITVendor E on VC.VendorID = E.VendorID "
        tmp = tmp & " Inner Join  Mst_Location L on CD.LocationID = L.LocationID "
        tmp = tmp & " Inner Join Mst_ExpenseType ET on CD.ExpenseTypeId = ET.ExpenseTypeID "
...
If Option1.Value = True Then
    ...
    tmp = tmp & " E.NAVCode 'VendorNAVCode', VC.BatchID ,'' As InvoiceDate From  Txn_ITVendorClaim VC "
    tmp = tmp & " Inner Join Mst_ITVendor E on VC.VendorID = E.VendorID "
...
Else
   ...
   tmp = tmp & " '' As 'VendorNAVCode', VC.BatchID ,'' As InvoiceDate From  Txn_ITVendorClaim VC "
   tmp = tmp & " Inner Join Mst_ITVendor E on VC.VendorID = E.VendorID "
   tmp = tmp & " Inner Join Mst_TDS T on E.TDSID = T.TDSID  "
   tmp = tmp & " Where VC.BatchID =  " & Val(CmbBatch.Text) & " and  VC.Flag = 'A'"
   tmp = tmp & " Order By VC.ApprovalDate"
...
Call Grid_Head
tmp = "Select Distinct BatchID from Txn_ITVendorClaim Where BatchID > 0  order by BatchID"
Call Tmp4Table
CmbBatch.Clear
