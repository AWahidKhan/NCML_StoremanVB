...
Else
   wc = " Where MSC.BookedFrom <= '" & mToDate & "' And  MSC.ExtendedPeriod >= '" & mFromDate & "'"
   wc = wc & " And ContractID not in (Select ContractID from Txn_ReservationInvoice Where YearMonth = '" & mFromDate & "')"
   wc = wc & " And ContractID not in (Select ContractID from Txn_ReservationInvoice Where YearMonth = '" & mFromDate & "' and (ReverseFlag <> 'Y'))"
...
'tmp = "Update Txn_ReservationInvoice Set BillingAmount= (select Sum(isnull(Amount,0)) from Txn_ReservationInvoiceDetail Where InvoiceNo=" & mInvNo & ")" 'Change released on 27-12-2024 by Abdul Wahid Khan - Start
tmp = "Update Txn_ReservationInvoice Set BillingAmount= (select Sum(isnull(Amount,0)) from Txn_ReservationInvoiceDetail Where InvoiceNo=" & mInvNo & "), ActualAmount= (select Sum(isnull(Amount,0)) from Txn_ReservationInvoiceDetail Where InvoiceNo=" & mInvNo & ")" 'Change released on 27-12-2024 by Abdul Wahid Khan
...
If LCase(mBookedAsPer) <> "fixed" Then
   ...
   mNarration = "Storage Reservation charge for " & mCommodityName & "" 'Change released on 27-12-2024 by Abdul Wahid Khan
Else
...
'Change released on 27-12-2024 by Abdul Wahid Khan - Start

'GenerateRemarks = mNarration

tmp = "Select distinct a.InvoiceNo,a.ContractID,f.GodownID,f.UnitTypeID from Txn_ReservationInvoice a"
tmp = tmp & " Inner Join Mst_StorageContract c on a.ContractID = c.ContractID"
tmp = tmp & " Inner Join Txn_SRGodownMapping d on c.ContractID = d.ContractID"
tmp = tmp & " Inner Join Txn_SRGodownMapDetails e on d.SRGodownMappingID = e.SRGodownMappingID"
tmp = tmp & " Inner Join Mst_Godown f on f.GodownID = e.GodownID"
tmp = tmp & " Where a.InvoiceNO = " & mInv_ & ""
...
For x = 1 To TmpRs.RecordCount
    ...
    If TmpRs!UnitTypeID = 1 Then
        ...
        tmp = "select distinct LessorName from Txn_LessorAgreement a"
        tmp = tmp & " Inner Join Txn_LessorAgreementDetail b on a.LAID = b.LAID"
        tmp = tmp & " Inner Join Txn_GodownLessorAgreeDetail c on b.LAID = c.LAID"
        tmp = tmp & " Inner Join Mst_Lessor d on b.LessorID = d.LessorID"
        tmp = tmp & " Where c.GodownID = " & TmpRs!GodownID & ""
        
        Call Tmp1Table
        
        If TmpRs1.RecordCount > 0 Then
           mLessorName = TmpRs1!LessorName
        End If
        
        If (mLessorName <> "") Then
            If (mWarehouseName = "") Then
                mWarehouseName = mLessorName
            Else
                mWarehouseName = mWarehouseName & ", " & mLessorName & "  "
            End If
        End If
    
    ElseIf TmpRs!UnitTypeID = 2 Then
    
        tmp = "select distinct d.VendorName from Mst_FF_RateCard_Godown a"
        tmp = tmp & " Inner Join Mst_FF_RateCard b on a.FF_RateCardID = b.FF_RateCardID"
        tmp = tmp & " Inner Join Mst_FF_RateCard_VendorMapping c on b.FF_RateCardID = c.FF_RateCardID"
        tmp = tmp & " Inner Join Mst_Vendor d on d.VendorID = c.VendorID"
        tmp = tmp & " Where a.GodownID = " & TmpRs!GodownID & ""
    
        Call Tmp1Table
        
        If TmpRs1.RecordCount > 0 Then
           mVendorName = TmpRs1!VendorName
        End If
        
        If (mVendorName <> "") Then
            If (mWarehouseName = "") Then
                mWarehouseName = mVendorName
            Else
                mWarehouseName = mWarehouseName & ", " & mVendorName & "  "
            End If
        End If
    ...
    ElseIf TmpRs!UnitTypeID = 4 Then
        ...
        tmp = "select SUBSTRING(b.Address, 1, 20) WarehouseAddr from Mst_Godown a"
        tmp = tmp & " Inner Join Mst_CMP b on a.CMPID = b.CMPID"
        tmp = tmp & " Where a.GodownID = " & TmpRs!GodownID & ""

        Call Tmp1Table

        If TmpRs1.RecordCount > 0 Then
           mVendorName = TmpRs1!WarehouseAddr
        End If
    ...
    ElseIf TmpRs!UnitTypeID = 7 Or TmpRs!UnitTypeID = 8 Then
    
        mWarehouseName = mWarehouseName & " NCML Project "
...
GenerateRemarks = mNarration & " (" & mWarehouseName & ")"

'Change released on 27-12-2024 by Abdul Wahid Khan - End
