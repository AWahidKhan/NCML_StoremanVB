...
tmp = tmp & " Sum(TCG.GrossWeight) as 'Weight','Quantity' Unit,MNRC.Rate, (datediff(day,MG.CDTFDate,TCD.CxTFDate) + 1) * Sum(TCG.DematWeight) * Rate as 'Amount'"
...
tmp = tmp & " Left Join Mst_NCDEXStorageRateCard MNR On ML.LocationID = MNR.LocationID And MCG.CommodityGroupID = MNR.CommodityGroupID"
...
If TxtSCxTFNo <> "" Then
   wc = wc & " And TCD.CxTFNo = " & Val(TxtSCxTFNo) & ""
End If
...
tmp1 = tmp1 & " MGod.GodownNo +'~'+ MG.StackNo  +'~'+ MG.LotNo,MG.CDTFDate,MNRC.Rate"
...
For i = 1 To TmpRs.RecordCount
    ...
    MSHFlexGrid1.TextMatrix(i, 8) = Format(MSHFlexGrid1.TextMatrix(i, 8), "dd-mmm-yyyy")
    TmpRs.MoveNext
    
    If (Val(MSHFlexGrid1.TextMatrix(i, 14)) = 0) Then
       For c = 0 To MSHFlexGrid1.Cols - 1
           MSHFlexGrid1.Col = c
           MSHFlexGrid1.Row = i
