...
Begin VB.Form Frm_ExportReverseCommtrackInvoice 
   Caption         =   "Export Reverse Commtrack Invoices"
   ClientHeight    =   3195
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   4680
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox CmbInvoiceType 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "Frm_ExportReverseCommtrackInvoice.frx":0000
         Left            =   5760
         List            =   "Frm_ExportReverseCommtrackInvoice.frx":000A
         Sorted          =   -1  'True
         TabIndex        =   3
         Top             =   240
         Width           =   2895
   ...
   End
End
Attribute VB_Name = "Frm_ExportReverseCommtrackInvoice"
...
CmbInvoiceType.Text = ""
tmp = "select distinct RBatchID As RBatchID from Txn_MonthlyCommtrackInvoice Where RBatchID > 0  order by RBatchID"
Call Tmp1Table
CmbBatch.Clear
...
If Option1.Value = True Then
   If CmbInvoiceType.Text = "Non Taxable" Then
      tmp = "Select * From Txn_MonthlyCommtrackInvoice Where RBatchID = 0 And Flag = 'N' And ReverseFlag = 'Y' "
   Else
      tmp = "Select * From Txn_MonthlyCommtrackInvoice Where RBatchID = 0  And Flag = 'T' And ReverseFlag = 'Y' "
   End If
   Call Tmp1Table
End If
With MSHFlexGrid3
...
'        Print #1, tmp
        ...
        If Option1.Value = True Then
            If TmpRs1.RecordCount > 0 Then
               TmpRs1.MoveFirst
               TmpRs1.Find "PINo = " & Val(Replace(MSHFlexGrid3.TextMatrix(I, 13), "RCommtrack", "")) & " "
...
'
'   If CmbInvoiceType.Text = "Non Taxable" Then
'      tmp = "Update Txn_MonthlyCommtrackInvoice set BatchID = " & MaxID & " Where BatchID is Null And Flag='N' "
'   Else
'      tmp = "Update Txn_MonthlyCommtrackInvoice set BatchID = " & MaxID & " Where BatchID is Null And Flag='T' "
'   End If
'   Call TmpTable
'End If
...
tmp = "select Max(RBatchID) from Txn_MonthlyCommtrackInvoice"
Call Tmp1Table
If TmpRs1.RecordCount > 0 Then
...
'mGLAccNAVCode = GetGLAccNAVCode("Commtrack Taxable")
'mSTCode = GetGLAccNAVCode("CommtrackSTCode")
mIncomeTypeID = 12
...
tmp = "Select MD.InvoiceID,MD.MonthYear,MD.LocationID,MDGST.GSTNavCode 'CPNAVCode',MD.Amt,"
tmp = tmp & " MD.STAmt , MD.TotalAmt, MD.PINo, MD.BatchID, ML.NAVCode, MD.Flag,ML.LocationName,MD.ReverseDate,MD.ReverseRemark,MD.HSNNo,MD.AccountType,MD.GSTGroupCode "
tmp = tmp & " From Txn_MonthlyCommtrackInvoice MD"
...
tmp = tmp & " Left Join Mst_Depository MD1 on MD.CPID = MD1.DepositoryID"
tmp = tmp & " Left Join Mst_DepositoryStateWiseGST MDGST On MD1.DepositoryID = MDGST.DepositoryID And MSI.StateIndiaID = MDGST.StateIndiaID "
tmp = tmp & wc
...
'       MSHFlexGrid3.TextMatrix(I + 1, 3) = IIf(IsNull(TmpRs!CPNAVCode), "", TmpRs!CPNAVCode)
...
'          'MSHFlexGrid3.TextMatrix(I + 1, 12) = "Being Storage & Warehousing Charges  (Commtrack) for " & TmpRs!LocationName & " for the month of " & MonthName(Month(TmpRs!MonthYear)) & " " & Year(TmpRs!MonthYear) & " (Without ST) "
'
'          MSHFlexGrid3.TextMatrix(I + 1, 12) = IIf(IsNull(TmpRs!ReverseRemark), "", Trim(TmpRs!ReverseRemark)) '"Being Reversal of Storage & Warehouse charges for the Commtrack/NCDEX clients at " & TmpRs!LocationName & " for the month of " & MonthName(Month(TmpRs!MonthYear)) & " " & Year(TmpRs!MonthYear) & " (Without ST) "
'       Else
'          MSHFlexGrid3.TextMatrix(I + 1, 19) = mSTCode
'          'Commented On 05 May 2012
'          'MSHFlexGrid3.TextMatrix(I + 1, 12) = "Being Storage & Warehousing Charges  (Commtrack) for " & TmpRs!LocationName & " for the month of " & MonthName(Month(TmpRs!MonthYear)) & " " & Year(TmpRs!MonthYear) & " (With ST) "
'
'          MSHFlexGrid3.TextMatrix(I + 1, 12) = IIf(IsNull(TmpRs!ReverseRemark), "", Trim(TmpRs!ReverseRemark)) '"Being Reversal of Storage & Warehouse charges for the Commtrack/NCDEX clients at " & TmpRs!LocationName & " for the month of " & MonthName(Month(TmpRs!MonthYear)) & " " & Year(TmpRs!MonthYear) & " (With ST) "
'          'Being Storage & Warehouse charges for the Commtrack/NCDEX clients at _____(location) for the month of _____2012 (with ST)
'       End If
...
'
'       If IsNull(TmpRs!CPNAVCode) = True Or TmpRs!CPNAVCode = "" Then
'          ExpFlag = "N"
'       End If
...
If TmpRs.RecordCount > 0 Then
   ...
   For r = 1 To TmpRs.RecordCount
       ...
       MSHFlexGrid3.TextMatrix(I, 3) = IIf(IsNull(TmpRs!CPNAVCode), "", TmpRs!CPNAVCode)
       MSHFlexGrid3.TextMatrix(I, 17) = "G/L Account"
       MSHFlexGrid3.TextMatrix(I, 18) = mGLAccNAVCode
       
       MSHFlexGrid3.TextMatrix(I, 4) = TmpRs!Amt '-(TmpRs!Amt)
       'MSHFlexGrid3.TextMatrix(I, 5) = TmpRs!InvoiceNo
       MSHFlexGrid3.TextMatrix(I, 7) = TmpRs!NavCode
       MSHFlexGrid3.TextMatrix(I, 8) = "CMP"
       MSHFlexGrid3.TextMatrix(I, 13) = "RCommtrack" & TmpRs!PINo
       tmp = "Select EntryNo,NAVCode from Txn_TIDetails Where  PIFlag='Commtrack' And PIID=" & TmpRs!PINo & ""
       Call Tmp3Table
       ...
       End If
       MSHFlexGrid3.TextMatrix(I, 19) = mSTCode
       MSHFlexGrid3.TextMatrix(I, 20) = mHSNNo
       MSHFlexGrid3.TextMatrix(I, 21) = mInvoiceType
